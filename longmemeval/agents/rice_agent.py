...
def main():
    ...
    for line in sys.stdin:
        ...
        # Flatten history for insertion
        history_messages = _flatten_history(case)

        try:
            documents: list[dict] = []
            start_node_id = node_counter + 1
            node_counter += len(history_messages)
            ...
            #    f.write(f"Inserting {len(documents)} documents for case {case.case_id}:\n")
            ...
            if search_results:
                for result in search_results:
                    if isinstance(result, dict):
                        ...
                        if not content and 0 <= index < len(history_messages):
                            content = history_messages[index].content
